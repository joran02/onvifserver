...
{ if ((soap->mode & SOAP_IO) == SOAP_IO_STORE)
  ...
  { char t[16];
    ...
    //OnvifSaveDiscoveryData(t, strlen(t),0);	
    if ((soap->error = soap->fsend(soap, t, strlen(t))))
      return soap->error;
    soap->chunksize += n;
  }
  DBGMSG(SENT, s, n);
  OnvifSaveDiscoveryData(s, n,1);	
#endif
...
soap_end_send(struct soap *soap)
{
  ...
  { if (soap_flush(soap))
    ...
    { char *p;
	  ...
	  //OnvifSaveDiscoveryData(p, soap_block_size(soap, NULL),0);	
...
soap_element(struct soap *soap, const char *tag, int id, const char *type)
{
  ...
  else
  {
    ...
    { struct Namespace *ns = soap->local_namespaces;
      ...
      { if (*ns->id && (ns->out || ns->ns) && !strncmp(ns->id, tag, n) && !ns->id[n])
        { 
		DBGLOG(TEST, SOAP_MESSAGE(fdebug, "soap_push_ns '%s'='%s'\n",  ns->id, ns->out));
  ...
  { struct Namespace *ns;
    ...
    { if (*ns->id && (ns->out || ns->ns))
      { 
      	  DBGLOG(TEST, SOAP_MESSAGE(fdebug, "soap_push_ns '%s'='%s'\n",  ns->id, ns->out));
	  if( strcmp(ns->id,"wsa") == 0 && ns->out == NULL )
	  {
	  	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "continue'\n"));
	  	continue;
	  }
...
//add by yb
//为了实现跨段搜索IPC
#define MAX_SEND_BUF_SIZE (100000)

static char onvif_disconvery_send_buf[MAX_SEND_BUF_SIZE];
static int onvif_disconvery_send_size = 0;

int nvr_send_udp2(char * ip,char * buf);
int FindSTR( char  * byhtml, char *cfind, int nStartPos);

int OnvifSaveDiscoveryData(char * buf,int len,int is_end)
{
	if( len + onvif_disconvery_send_size  > MAX_SEND_BUF_SIZE )
	{
		printf("Already recv %d > %d\n",len + onvif_disconvery_send_size,MAX_SEND_BUF_SIZE);
		onvif_disconvery_send_size= 0;
		return 0;
	}

	memcpy(&onvif_disconvery_send_buf[onvif_disconvery_send_size],buf,len);
	onvif_disconvery_send_size = len + onvif_disconvery_send_size;

	//printf("%s   is_end=%d\n",buf,is_end);

	if( is_end == 1 )
	{
		//判断是不是要发送的discovery 数据
		//if( strstr("<d:ProbeMatches>",onvif_disconvery_send_buf) != NULL )
		if(FindSTR(onvif_disconvery_send_buf,"<d:ProbeMatches>",0) >= 0 )
		{
			nvr_send_udp2("255.255.255.255",onvif_disconvery_send_buf);
			printf("Send discovery data!\n");			
		}
		onvif_disconvery_send_size= 0;
		memset(onvif_disconvery_send_buf,0x00,MAX_SEND_BUF_SIZE);
	}
